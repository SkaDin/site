    ...
    with open("data_for_start.csv", encoding="utf-8") as f:
        ...
        for other_row in readers:
            ...
            timestamp = datetime.strptime(
                other_row["timestamp"], "%Y-%m-%d%H:%M:%S.%f"  # noqa
            )
...
def add_posts():
    ...
    if form.validate_on_submit():
        ...
        if (
            Post.query.filter_by(text=text).first() is not None
            and Post.query.filter_by(title=title).first() is not None
        ):
            flash(
                f"Проверьте поля с названием и описанием"
                f" - они могут быть не уникальными!"
            )
            return render_template("add_post.html", form=form)
        filename = secure_filename(str(image))
        image_file = request.files["image"]
        image_file.save(os.path.join(Config.PEOPLE_FOLDER, filename))
        post = Post(
            title=form.title.data,
            image=filename,
            text=form.text.data,
...
def users_posts(user_id: int):  # noqa
    ...
    posts = Post.query.filter_by(user_id=user_id).paginate(
        page=page, per_page=TWELVE
    )
    return render_template(
        "user_posts.html", posts=posts, selected_user=selected_user
    )
